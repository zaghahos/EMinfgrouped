...
dataEM_n1500_bin10<- read.csv("C:/Users/sh_za/OneDrive/Desktop/Results/Bivariate/EM Oct2021/EM1500b10.csv")
dataEM_n1500_bin10<- dataEM_n1500_bin10[,-1]
dataEM_n1500_bin10
EM1500b10
...
Std_Mu_Sigma_EM<- function(Data,mu,sigma){ 
  ...
  for(i in 1:nrow(dS_mu)){
    ...
    #dS_sigma[,,i]<- solve(sigma)+(0.5)*diag(solve(sigma))+solve(sigma)%*%CovX[,,i]%*%solve(sigma)-
     # (0.5)*diag(solve(sigma)%*%CovX[,,i]%*%solve(sigma))
    

    
    
    dS_sigma[,,i]<- sigma-(0.5)*diag(sigma)-sigma%*%CovX[,,i]%*%sigma+
      (0.5)*diag(sigma%*%CovX[,,i]%*%sigma)
...
#################################################################################################
EM1500b10<- dataEM_n1500_bin10
EM1500b10

mu_EM1500b10<- matrix(rep(0,2*nrow(EM1500b10)),ncol=2)
for(i in 1:nrow(EM1500b10)){
  mu_EM1500b10[i,]<- c(EM1500b10[i,1],EM1500b10[i,2])
}
mu_EM1500b10


sigma_EM1500b10<- array(rep(0,2*2*nrow(EM1500b10)),c(2,2,nrow(EM1500b10)))
#sigma_EM1000b10
for(i in 1:nrow(EM1500b10)){
  sigma_EM1500b10[,,i]<- matrix(c(EM1500b10[i,3],EM1500b10[i,4],EM1500b10[i,5],EM1500b10[i,6]),ncol=2)
}
sigma_EM1500b10
#######################################################################################################
...
result1<- Std_Mu_Sigma_EM(Data=simulateddata1500b10[,,1],mu=mu_EM1500b10[1,],sigma=sigma_EM1500b10[,,1])

result1


result2<- Std_Mu_Sigma_EM(Data=simulateddata1000b10[,,1],mu=mu_EM1000b10[1,],sigma=sigma_EM1000b10[,,1])
