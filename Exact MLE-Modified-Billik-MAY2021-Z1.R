...
Billik<- function(Data,theta){ ## defining the log-likelihood function, The function has two arguments, Data: bivariate grouped
  ...
  sig<- matrix(subtheta,ncol=d,nrow=d,byrow=T) # Replace these elements in sigma matrix
  print(sig)
...
sigma11<- matrix(c(1.760682, 0.4987647, 0.4987641, 2.459675), 2, 2)

theta1<- c(mu1,as.vector(sigma11))
theta11<- c(mu1,as.vector(sigma1))

#theta2<- c(mu2,as.vector(sigma2))

MLEExactBiv<- optim(theta<- c(67,67,3.10,6.05,2.16),fn=Billik,Data=simulateddata1000b10[,,1],method="Nelder-Mead") ## use optim function with the same arguments described
MLEExactBiv$par ## to see the Exact MLE of the parameters


####n=1000####
#### MCEM1000b10: MEEM RESULTS FOR N=1000 & Binns=10
MLEBINNED1000b10<- matrix(rep(0*6*10),ncol=6,nrow=10)
colnames(MLEBINNED1000b10)<- c("mux1","mux2","Varx1","COVx1x2","COVx2x1","Varx2")
