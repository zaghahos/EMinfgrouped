...
VBiasMCEM[,1]<- outputMCEM50m8[,2]-3.24
VBiasMCEM[,2]<- outputMCEM50m15[,2]-3.24
VBiasMCEM[,3]<- outputMCEM50m30[,2]-3.24

VBiasMCEM[,4]<- outputMCEM100m8[,2]-3.24
VBiasMCEM[,5]<- outputMCEM100m15[,2]-3.24
VBiasMCEM[,6]<- outputMCEM100m30[,2]-3.24

VBiasMCEM[,7]<- outputMCEM300m8[,2]-3.24
VBiasMCEM[,8]<- outputMCEM300m15[,2]-3.24
VBiasMCEM[,9]<- outputMCEM300m30[,2]-3.24

VBiasMCEM[,10]<- outputMCEM600m8[,2]-3.24
VBiasMCEM[,11]<- outputMCEM600m15[,2]-3.24
VBiasMCEM[,12]<- outputMCEM600m30[,2]-3.24

VBiasMCEM[,13]<- outputMCEM1000m8[,2]-3.24
VBiasMCEM[,14]<- outputMCEM1000m15[,2]-3.24
VBiasMCEM[,15]<- outputMCEM1000m30[,2]-3.24
...
VMSEMCEM[,1]<- (outputMCEM50m8[,2]-3.24)^2
VMSEMCEM[,2]<- (outputMCEM50m15[,2]-3.24)^2
VMSEMCEM[,3]<- (outputMCEM50m30[,2]-3.24)^2

VMSEMCEM[,4]<- (outputMCEM100m8[,2]-3.24)^2
VMSEMCEM[,5]<- (outputMCEM100m15[,2]-3.24)^2
VMSEMCEM[,6]<- (outputMCEM100m30[,2]-3.24)^2

VMSEMCEM[,7]<- (outputMCEM300m8[,2]-3.24)^2
VMSEMCEM[,8]<- (outputMCEM300m15[,2]-3.24)^2
VMSEMCEM[,9]<- (outputMCEM300m30[,2]-3.24)^2

VMSEMCEM[,10]<- (outputMCEM600m8[,2]-3.24)^2
VMSEMCEM[,11]<- (outputMCEM600m15[,2]-3.24)^2
VMSEMCEM[,12]<- (outputMCEM600m30[,2]-3.24)^2

VMSEMCEM[,13]<- (outputMCEM1000m8[,2]-3.24)^2
VMSEMCEM[,14]<- (outputMCEM1000m15[,2]-3.24)^2
VMSEMCEM[,15]<- (outputMCEM1000m30[,2]-3.24)^2
...
VBiasEM[,1]<- output50m8EM[,2]-3.24
VBiasEM[,2]<- output50m15EM[,2]-3.24
VBiasEM[,3]<- output50m30EM[,2]-3.24

VBiasEM[,4]<- output100m8EM[,2]-3.24
VBiasEM[,5]<- output100m15EM[,2]-3.24
VBiasEM[,6]<- output100m30EM[,2]-3.24

VBiasEM[,7]<- output300m8EM[,2]-3.24
VBiasEM[,8]<- output300m15EM[,2]-3.24
VBiasEM[,9]<- output300m30EM[,2]-3.24

VBiasEM[,10]<- output600m8EM[,2]-3.24
VBiasEM[,11]<- output600m15EM[,2]-3.24
VBiasEM[,12]<- output600m30EM[,2]-3.24

VBiasEM[,13]<- output1000m8EM[,2]-3.24
VBiasEM[,14]<- output1000m15EM[,2]-3.24
VBiasEM[,15]<- output1000m30EM[,2]-3.24
...
VMSEEM[,1]<- (output50m8EM[,2]-3.24)^2
VMSEEM[,2]<- (output50m15EM[,2]-3.24)^2
VMSEEM[,3]<- (output50m30EM[,2]-3.24)^2

VMSEEM[,4]<- (output100m8EM[,2]-3.24)^2
VMSEEM[,5]<- (output100m15EM[,2]-3.24)^2
VMSEEM[,6]<- (output100m30EM[,2]-3.24)^2

VMSEEM[,7]<- (output300m8EM[,2]-3.24)^2
VMSEEM[,8]<- (output300m15EM[,2]-3.24)^2
VMSEEM[,9]<- (output300m30EM[,2]-3.24)^2

VMSEEM[,10]<- (output600m8EM[,2]-3.24)^2
VMSEEM[,11]<- (output600m15EM[,2]-3.24)^2
VMSEEM[,12]<- (output600m30EM[,2]-3.24)^2

VMSEEM[,13]<- (output1000m8EM[,2]-3.24)^2
VMSEEM[,14]<- (output1000m15EM[,2]-3.24)^2
VMSEEM[,15]<- (output1000m30EM[,2]-3.24)^2
...
VBiasMLEBin[,1]<- MLEbinned50m8[,2]-3.24
VBiasMLEBin[,2]<- MLEbinned50m15[,2]-3.24
VBiasMLEBin[,3]<- MLEbinned50m30[,2]-3.24

VBiasMLEBin[,4]<- MLEbinned100m8[,2]-3.24
VBiasMLEBin[,5]<- MLEbinned100m15[,2]-3.24
VBiasMLEBin[,6]<- MLEbinned100m30[,2]-3.24

VBiasMLEBin[,7]<- MLEbinned300m8[,2]-3.24
VBiasMLEBin[,8]<- MLEbinned300m15[,2]-3.24
VBiasMLEBin[,9]<- MLEbinned300m30[,2]-3.24

VBiasMLEBin[,10]<- MLEbinned600m8[,2]-3.24
VBiasMLEBin[,11]<- MLEbinned600m15[,2]-3.24
VBiasMLEBin[,12]<- MLEbinned600m30[,2]-3.24

VBiasMLEBin[,13]<- MLEbinned1000m8[,2]-3.24
VBiasMLEBin[,14]<- MLEbinned1000m15[,2]-3.24
VBiasMLEBin[,15]<- MLEbinned1000m30[,2]-3.24
...
VMSEMLEBin[,1]<- (MLEbinned50m8[,2]-3.24)^2
VMSEMLEBin[,2]<- (MLEbinned50m15[,2]-3.24)^2
VMSEMLEBin[,3]<- (MLEbinned50m30[,2]-3.24)^2

VMSEMLEBin[,4]<- (MLEbinned100m8[,2]-3.24)^2
VMSEMLEBin[,5]<- (MLEbinned100m15[,2]-3.24)^2
VMSEMLEBin[,6]<- (MLEbinned100m30[,2]-3.24)^2

VMSEMLEBin[,7]<- (MLEbinned300m8[,2]-3.24)^2
VMSEMLEBin[,8]<- (MLEbinned300m15[,2]-3.24)^2
VMSEMLEBin[,9]<- (MLEbinned300m30[,2]-3.24)^2

VMSEMLEBin[,10]<- (MLEbinned600m8[,2]-3.24)^2
VMSEMLEBin[,11]<- (MLEbinned600m15[,2]-3.24)^2
VMSEMLEBin[,12]<- (MLEbinned600m30[,2]-3.24)^2

VMSEMLEBin[,13]<- (MLEbinned1000m8[,2]-3.24)^2
VMSEMLEBin[,14]<- (MLEbinned1000m15[,2]-3.24)^2
VMSEMLEBin[,15]<- (MLEbinned1000m30[,2]-3.24)^2
...
#################################################################################################
cbind(VRMSEMLEUNBin,VRMSEMLEBin,VRMSEEM,VRMSEMCEM)
A<- rbind(VRMSEMLEUNBin,VRMSEMLEBin,VRMSEEM,VRMSEMCEM)
A
library(xtable)
xtable(A,digits = 6)
...
XX2<- c(Z2,Z5,Z8,Z11)
XX3<- c(Z3,Z6,Z9,Z12)
methods<- c(rep("MLE ignore grouping",5),rep("MLE Exact",5),rep("EM",5),rep("MCEM",5))
n<- rep(c(50,100,300,600,1000),4)
...
xtable(MeanRMSE)
####################################################################################y
Y1<- c(VRMSEMLEUNBin[1],VRMSEMLEUNBin[4],VRMSEMLEUNBin[7],VRMSEMLEUNBin[10],VRMSEMLEUNBin[13])
Y1
Y2<- c(VRMSEMLEUNBin[2],VRMSEMLEUNBin[5],VRMSEMLEUNBin[8],VRMSEMLEUNBin[11],VRMSEMLEUNBin[14])
Y2
Y3<- c(VRMSEMLEUNBin[3],VRMSEMLEUNBin[6],VRMSEMLEUNBin[9],VRMSEMLEUNBin[12],VRMSEMLEUNBin[15])
Y3
